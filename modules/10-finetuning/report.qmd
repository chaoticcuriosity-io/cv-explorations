...
Every module so far *ran* a **pretrained** model (one whose internal numbers — its *weights* — were
already learned and saved in a file you load). This one **trains** one — the capstone skill,
because real projects almost always need a class the off-the-shelf models don't have. (Two words
worth pinning down: *training* adjusts a model's internal numbers from labelled examples, while
*inference* just runs the finished model. Every earlier module did inference; this one does
training.) Here that
class is **air vents**: not in COCO, so [module 01's](../01-object-detection/report.qmd) detectors
are blind to it. We **fine-tune** a YOLO detector to find vents — fine-tuning means continuing to
train an already-pretrained model on your own smaller dataset to specialise it, which is far
cheaper than training from scratch. **YOLO** (here YOLO11, a small, fast object detector that draws
boxes around things; see [Model families](../../models.qmd)) is what we specialise. And — the
interesting part — we do it
**without hand-labelling a single image**, then train **on the DGX Spark**.

## The data engine: this repo, labelling itself

Fine-tuning needs labelled data — and a *label* (or *annotation*) is just the correct answer
attached to a training image; here, a box drawn around a vent. Labelling is the expensive, boring
bottleneck of applied CV.
...
[module 01](../01-object-detection/report.qmd) promised. (*Distillation* means training a small,
fast model to copy a big, slow one's outputs. *Open-vocabulary* — or *zero-shot* — means a model
that detects or classifies things from a text *description* rather than from a fixed list it was
trained on, which is exactly how CLIP and Grounding DINO are used below to build the data.) The
recipe:

1. **Source** — pull Creative-Commons vent images from Wikimedia Commons (no API key).
2. **Filter (zero-shot)** — score each image with **CLIP** (a model that scores how well an image
   matches a text description; [module 02](../02-classification/report.qmd), and
   [Model families](../../models.qmd))
...
3. **Auto-label** — run **Grounding DINO** (an open-vocabulary detector that draws boxes from a text
   prompt; [module 01](../01-object-detection/report.qmd)) with vent
   text prompts; its boxes (after a size-sanity filter) become YOLO labels. Because *another* model
   produces them rather than a human, these are **pseudo-labels** — cheap to generate, but noisier
...
below (**mAP** — mean Average Precision — is the standard detection score, from 0 to 1, where
higher is better; the "50" in *mAP50* refers to the box-overlap threshold used to count a
detection as correct) measures *how well YOLO distilled Grounding DINO*, not human ground truth. The qualitative
...
An **epoch** is one full pass of training over the whole dataset; the curves below span many such
passes.
